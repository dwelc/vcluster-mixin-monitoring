...
//
// Split into controlPlaneRules (apiserver-derived, always emitted) and workloadRules
// (cAdvisor-derived, only emitted when scrapeMode != 'control-plane').
{
  local controlPlaneRules = [
    {
  ...
  prometheusRules+:: {
    groups+: [
      {
        name: 'vcluster.rules',
        rules: (if $._config.scrapeMode == 'workload' then [] else controlPlaneRules) +
               (if $._config.scrapeMode == 'control-plane' then [] else workloadRules),
      },
    ],
  },
}
